...
public class AccountingController {
    ...
    public AOP addAOP(@RequestBody AOP aop) {
        ...
        List<Account> accounts = accountRepo.findByUserIdAndAopCodeAndAopListYear(
            aop.getUserId(), 
            aop.getAopCode(), 
            aop.getAopListYear()
        );
        ...
        if (!accounts.isEmpty()) {
            for (Account account : accounts) {
                account.setAopName(aop.getAopName());
            }
    ...
    public List<Invoice> getInvoices() {
        return invoiceRepo.findAllWithItems();
    }
    ...
    public ResponseEntity<Invoice> updateInvoice(
        ...
        String oldInvoiceNumber = inv.getInvoiceNumber();
        String oldType = inv.getType().name();
        String oldUserId = inv.getUserId();
    ...
    public Account updateAccount(@PathVariable String id, @RequestBody Account acc) {
        ...
        if (acc.getCode().startsWith("120") && acc.getCode().length()<=4 ||
            ...
            if (acc.getCode().startsWith("120") || acc.getCode().startsWith("121") || acc.getCode().startsWith("220") || acc.getCode().startsWith("121")) {
                for (Invoice inv : invoiceRepo.findAll()) {
                    if (inv.getAccount().equals(acc2.getCode()) && inv.getDate().contains(Integer.toString(acc2.getAopListYear())) && inv.getUserId().equals(acc2.getUserId())) {
                        List<Invoice> listInv = invoiceRepo.findByInvoiceNumberAndUserIdAndType(inv.getInvoiceNumber(), inv.getUserId(), inv.getType());
                        for (Invoice inv2 : listInv) {
                ...
                for (GiroAccount giro : giroAccountRepo.findAll()) {
                    if (giro.getOutgoingAccount().equals(acc2.getCode()) && giro.getDate().contains(Integer.toString(acc2.getAopListYear())) && giro.getUserId().equals(acc2.getUserId())) {
                        List<GiroAccount> listGiro = giroAccountRepo.findByInvoiceNumberAndDescriptionAndUserId(giro.getInvoiceNumber(), giro.getDescription(), giro.getUserId());
                        for (GiroAccount giro2 : listGiro) {
                ...
                for (LedgerEntry ledger : ledgerRepo.findAll()) {
                    if (ledger.getAccountCode().equals(acc2.getCode()) && ledger.getDate().contains(Integer.toString(acc2.getAopListYear())) && ledger.getUserId().equals(acc2.getUserId())) {
    ...
    public List<LedgerEntry> updateBatch(
            @RequestParam String oldInvoiceNumber,
            @RequestBody List<LedgerEntry> entries
    ) {
        ...
        String invoiceNumber = oldInvoiceNumber;
        ...
        for (LedgerEntry entry : entries) {
            entry.setId(null);
            entry.setStatus(InvoiceStatus.PRE_LEDGER);
        }
        ...
        // ako nema postojećih → samo spremi
        ...
        List<LedgerEntry> clean = new ArrayList<>();

        for (LedgerEntry e : entries) {
            LedgerEntry n = new LedgerEntry();

            n.setUserId(e.getUserId());
            n.setDate(e.getDate());
            n.setAccountCode(e.getAccountCode());
            n.setDebit(e.getDebit());
            n.setCredit(e.getCredit());
            n.setDescription(e.getDescription());
            n.setPartnerId(e.getPartnerId());
            n.setReferenceId(e.getReferenceId());
            n.setInvoiceNumber(e.getInvoiceNumber());
            n.setLedgerType(e.getLedgerType());
            n.setYear(e.getYear());

            n.setStatus(InvoiceStatus.PRE_LEDGER);

            clean.add(n);
        }

        return ledgerRepo.saveAll(clean);
    }
    ...
    @PostMapping("/map-balances")
    public ResponseEntity<?> mapBalances(
            @RequestParam String userId,
            @RequestParam Integer aopListYear) {
        ...
        try {
            ...
            return ResponseEntity.ok(
                    new MappingResponse(
                            true,
                            "Mapiranje računa uspješno završeno.",
                            userId,
                            aopListYear
                    )
            );
    ...
    @PostMapping("/aop/move-next-year")
    public ResponseEntity<?> moveAopToNextYear(
            @RequestParam String userId,
            @RequestParam Integer aopListYear) {

        try {

            service.moveAopToNextYear(
                    userId,
                    aopListYear
            );

            return ResponseEntity.ok(
                    new MappingResponseAop(
                            true,
                            "AOP podaci uspješno prebačeni iz "
                                    + aopListYear
                                    + " u "
                                    + (aopListYear + 1),
                            userId,
            ...
            return ResponseEntity.internalServerError().body(
                    new MappingResponseAop(
                            false,
                            "Greška kod prebacivanja AOP podataka: "
                                    + e.getMessage(),
                            userId,
                            aopListYear,
                            null
    ...
    public static class MappingResponse {
        ...
        public Integer getAopListYear() {
            return aopListYear;
        }

        public void setAopListYear(Integer aopListYear) {
            this.aopListYear = aopListYear;
        }
    }
    
    public static class MappingResponseAop {
    ...
    @DeleteMapping("/aop/deleteallinyear")
    public ResponseEntity<?> deleteByAopListYear(
            @RequestParam String userId,
            @RequestParam Integer aopListYear) {

        try {

            service.deleteByAopListYear(
                    userId,
                    aopListYear
            );
            ...
            return ResponseEntity.internalServerError()
                    .body(
                            "Greška kod brisanja AOP stavki: "
                                    + e.getMessage()
                    );
        }
    }    
    
    @DeleteMapping("/account/deleteallinyear")
    public ResponseEntity<?> deleteAccountByAopListYear(
            @RequestParam String userId,
            @RequestParam Integer aopListYear) {

        try {

            service.deleteAccountByAopListYear(
                    userId,
                    aopListYear
            );
            ...
            return ResponseEntity.internalServerError()
                    .body(
                            "Greška kod brisanja Konto stavki: "
                                    + e.getMessage()
                    );
        }
    }        
